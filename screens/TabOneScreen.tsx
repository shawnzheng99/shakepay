...
export default function TabOneScreen({ navigation }: RootTabScreenProps<'TabOne'>) {
  ...
  const renderListItem = ({ item }: {item: CryptoType}) => {
    ...
    const diffInPercentage = new Intl.NumberFormat('en-CA',{style: 'percent', minimumFractionDigits: 2,maximumFractionDigits: 6}).format(differences);
    const isUp = differences > 0

    return (
      <TouchableOpacity style={{height: 60, flexDirection: 'row', width: '100%', marginBottom: 20, }}>
...
const moneyFormatter = (moneyAmt: number) => 
